...
import { genericInternalError, M2MJwtDbTable } from "pagopa-interop-kpi-models";
...
describe("DB Service tests", () => {
  ...
  const dpopStagingTable = `${M2MJwtDbTable.dpop}${config.mergeTableSuffix}`;
  const clientAssertionStagingTable = `${M2MJwtDbTable.client_assertion}${config.mergeTableSuffix}`;
  const generatedTokenStagingTable = `${M2MJwtDbTable.generated_token}${config.mergeTableSuffix}`;
  const dpopTargetTable = `${M2MJwtDbTable.dpop}`;
  const clientAssertionTargetTable = `${M2MJwtDbTable.client_assertion}`;
  const generatedTokenTargetTable = `${M2MJwtDbTable.generated_token}`;
  ...
  describe("insertRecordsToStaging", () => {
    it("should insert records and commit the transaction successfully", async () => {
      const records: GeneratedApiTokenAuditDetails[] = getMockJwtAudits(10).map(
        (record, index) =>
          index === 0 ? { ...record, adminId: undefined } : record
      );
      ...
      for (const row of clientAssertionTable) {
        ...
        expect(row.audience).toBe("interop.pagopa.it/client-assertion");
      }

      const generatedTokenTable = await conn.query<GeneratedApiTokenSchema[]>(
        `SELECT * FROM $1:name;`,
        [generatedTokenStagingTable]
      );

      expect(
        generatedTokenTable.filter((row) => row.admin_id === null)
      ).toHaveLength(1);
      expect(
        generatedTokenTable.filter((row) => row.admin_id !== null)
      ).toHaveLength(9);
    ...
    it("should throw an error and rollback generated_token records when client_assertion inserts fails", async () => {
      ...
      const records: GeneratedApiTokenAuditDetails[] = getMockJwtAudits(10);
